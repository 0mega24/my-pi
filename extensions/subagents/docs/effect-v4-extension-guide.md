...
Promise-based SDK calls can be wrapped in `Effect.tryPromise` with a typed error. The
...
const fetchRecord = (client: SdkClient, id: string) =>
  Effect.tryPromise({
    try: (signal) => client.fetchRecord(id, { signal }),
    catch: (cause) => new SdkError({ cause }),
  });
```

There's no `Layer` here unless you want the client as a service. For a single tool this is
fine to call directly through `runTool` (or even plain `Effect.runPromiseExit`, §2). Keep
configuration reading and result truncation as plain TS unless they benefit from Effect.
